...
int count_valid_strings(int K, const std::vector<int>& C) {
    int n = 26;  // Number of uppercase English letters
    // dp[i][j] - Number of ways to form strings of length j using first i letters
    ...
    for (int i = 1; i <= n; ++i) {
        ...
        for (int length = 0; length <= K; ++length) {
            for (int count = 0; count <= c_i && length + count <= K; ++count) {
                dp[i][length + count] = (dp[i][length + count] + dp[i - 1][length]) % MOD;
            }
        }
    }
...
void solve(){
    ...
    generate(all(candy), nxt);
    generate(all(req), nxt);
