...
void solve(){
    ...
    // generate(all(cows), [](){int a, b; std::cin>>a>>b; return make_pair(a, b); });
    rep(i, 0, n){
        int a = nxt(), b = nxt();
        cows[i] = {a,b};
    }

    sort(all(cows), [](pii a, pii b){return a.S > b.S;} );
    int res = 0;
    rep(i, n, 0){
        // cout << i << endl;
        auto it = chickens.lower_bound(cows[i].F);
        if(it == chickens.end())
            continue;
        if(*it <= cows[i].S && *it >= cows[i].F) {
            res++;
            chickens.erase(it);
        }
    }
