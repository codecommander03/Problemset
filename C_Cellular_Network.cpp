...
void solve()
{
    int n = nxt(), m = nxt(), res = 2e9+7;
    vi cities(n), towers(m);
    generate(all(cities), nxt);
    generate(all(towers), nxt);

    cities.erase(unique(all(cities)), cities.end());
    towers.erase(unique(all(towers)), towers.end());

    n = sz(cities);
    m = sz(towers);
    // for(int i: cities) cout << i << " ";
    // cout << endl;
    // for(int i: towers) cout << i << " ";
    // cout << endl;
    // cout << n << endl;
    ...
    auto check = [&](int d)
    {
        ...
        while (i < n && j < m)
        {
            while (i < n && abs(towers[j] - cities[i]) <= d)
                i++;
            if (i == n)
                return true;
            j++;
