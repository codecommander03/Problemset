...
void solve(){
    ...
    int ans = 0;
    auto cal = [&](vector<char> ges)
    {
        int res = 0;
        rep(i, 0, n)
        {
            if(ges[games[i].F]=='R' && ges[games[i].S]=='S')
                res++;
            else if (ges[games[i].F] == 'P' && ges[games[i].S] == 'R')
                res++;
            else if (ges[games[i].F] == 'S' && ges[games[i].S] == 'P')
                res++;
        }
        return res;
    };
    ans = max({cal({'R', 'P', 'S'}), cal({'R', 'S', 'P'}), cal({'S', 'R', 'P'}), cal({'S', 'P', 'R'}), cal({'P', 'R', 'S'}), cal({'P', 'S', 'R'})});
    cout << ans << endl;
}
