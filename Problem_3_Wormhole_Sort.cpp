...
void dfs(vector<vpi> &graph, vi &visited, int node, int k, int color)
{
    visited[node] = color;
    for(auto &it: graph[node]){
        int child = it.F, weight = it.S;
        if(weight < k) continue;
        if(visited[child] != -1)
            continue;
        
        dfs(graph, visited, child, k, color);
    }
}

bool binarySearch(vector<vpi>&graph, vi& cows, int k){
    int n = sz(graph);
    vi visited(n, -1);
    rep(i, 0, n){
...
void solve(){
    ...
    while ((b - a) > 1)
    {
        if (binarySearch(graph, cows, (a + b) / 2))
        {
            a += (b - a) / 2;
        }
        else
